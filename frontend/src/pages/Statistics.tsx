...
export function Statistics() {
  ...
  useEffect(() => {
    ...
    void api.request('statistics.report', {instance, category, days, month, period, series: researchScope === 'series' ? Number(researchSeries) : 0, scope: researchScope}).then(value => {if (active) setData(value)}).catch(error => {if (active) setError(error.message)})
    return () => {active = false}
  ...
  const silentRefresh = useCallback(() => {
    if (connection !== 'ready') return
    void api.request('statistics.report', {instance, category, days, month, period, series: researchScope === 'series' ? Number(researchSeries) : 0, scope: researchScope})
      .then(value => {
        setData(value)
      })
      .catch(() => {
