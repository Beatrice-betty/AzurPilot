...
| `OpsiHazard1Leveling.ExecuteFixedPatrolScan` | bool | 战后强制移动开关（默认 false）。开启后零移动遍历 1~4 队雷达找问号；仍找不到且**界面上的当前行动力**（不含药剂箱，`_read_current_action_point()`）大于 7 时，再逐队挪动舰队做一遍整图重扫。**短猫相接**跑这条兜底（重扫时看到明石却走不过去）不看行动力，直接挪。旧版等级 1/2/3 一并视为开启 |
